...
# %% U-Dice Loss weighed only on errors

def get_u_dice_loss_errors_only(class_weights=(0., 1.), use_mask=True, alpha=1.0, downweight_uncertainty=False):
    """
    Factory function for an Uncertainty-Aware Dice Loss where uncertainty 
    weights ONLY the prediction errors (FP and FN).
    ...
    def loss(y_true, y_pred, sigma=None):
        ...
        # 2. Compute and apply the spatial exclusion mask
        if use_mask:
            mask = tf.reduce_sum(y_true, axis=-1)
            mask = tf.cast(mask > 0, dtype=tf.float32)
            mask = tf.expand_dims(mask, axis=-1)
        ...
        # 3. Compute continuous True Positives, False Positives, and False Negatives
        tp = y_true * y_pred
        fp = (1.0 - y_true) * y_pred
        fn = y_true * (1.0 - y_pred)

        # 4. Apply Uncertainty SPATIAL Weight ONLY to Errors (FP and FN)
        if sigma is not None:
            sigma = tf.cast(sigma, tf.float32)
            if sigma.shape.rank == y_true.shape.rank:
                sigma = tf.squeeze(sigma, axis=-1) # Ensure shape (B, H, W)
            ...
            # Expand dimensions to broadcast against the class channel: (B, H, W) -> (B, H, W, 1)
            dice_weight_map = tf.expand_dims(tf.cast(dice_weight_map, tf.float32), axis=-1)
            
            # Weight FP and FN spatially, leaving TP unweighted
            fp = fp * dice_weight_map
            fn = fn * dice_weight_map

        # 5. Global Batch Reductions (Micro-Average over Batch, Height, Width)
        tp_sum = tf.reduce_sum(tp, axis=(0, 1, 2))
        fp_sum = tf.reduce_sum(fp, axis=(0, 1, 2))
        fn_sum = tf.reduce_sum(fn, axis=(0, 1, 2))
        
        # 6. Calculate Dice scores [Shape: (num_classes,)]
        # Equivalence: 2*TP / (2*TP + FP_weighted + FN_weighted)
        dice = (2.0 * tp_sum + smooth) / (2.0 * tp_sum + fp_sum + fn_sum + smooth)
