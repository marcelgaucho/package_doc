...
# -*- coding: utf-8 -*-
"""
Created on Thu Aug 27 16:34:12 2026
...
class MidpointPatchMerger:
    def __init__(self, target_shape, patch_size, overlap_percent=0.0):
    ...
    def __call__(self, patches, coords, dtype=np.uint8):
        ...
        for patch, (ymin, xmin, ymax, xmax) in zip(patches, coords):
            # 1. Generate mask on the fly (skipped entirely if overlap_percent = 0)
            ...
            img_ymin, img_xmin = max(0, ymin), max(0, xmin)
            img_ymax, img_xmax = min(self.h, ymax), min(self.w, xmax)
